...
class PostController extends Controller
{
    ...
    public function view(Request $request, Post $post)
    {
        if ($post->group_id && !$post->group->hasApprovedUser(Auth::id())) {
            return inertia('Error', [
                ...
                'body' => "Vous n'avez pas l'autorisation de voir cette publication"
    ...
    public function destroy(Post $post)
    {
        ...
        return response("Vous n'avez pas l'autorisation de supprimer cette publication", 403);
    }
    ...
    public function deleteComment(Comment $comment)
    {
        ...
        if ($comment->isOwner($id) || $post->isOwner($id)) {
            ...
            return response(['supprimé' => $deletedCommentCount], 200);
        }

        return response("Vous n'avez pas l'autorisation de supprimer ce commentaire", 403);
    ...
    public function aiPostContent(Request $request)
    {
        ...
        $result = OpenAI::chat()->create([
            ...
            'messages' => [
                [
                    'role' => 'user',
                    'content' => "Veuillez générer du contenu pour une publication sur les réseaux sociaux basé sur l'instruction suivante. Générer un contenu formaté avec plusieurs paragraphes. Ajouter les hashtags après deux lignes du contenu principal" . PHP_EOL . PHP_EOL . "Prompt: " . PHP_EOL
                        . $prompt
                ],
            ],
        ]);
...
//            'content' => "\"🎉 Exciting news! We're thrilled to announce that we just released a brand new feature on our app/website! 💥 Get ready to experience the next level of convenience and efficiency with this game-changing addition. 🚀 Try it out now and let us know what you think! 😍 #NewFeatureAlert #UpgradeYourExperience\""
    ...
    public function pinUnpin(Request $request, Post $post)
    {
        ...
        if ($forGroup && !$group) {
            return response("Demande invalide", 400);
        }

        if ($forGroup && !$group->isAdmin(Auth::id())) {
            return response("Vous n'avez pas l'autorisation d'effectuer cette action", 403);
        }
        ...
        return back()->with('success', 'La publication a été effectuée avec succès ' . ( $pinned ? 'pinned' : 'unpinned' ));
