...
//        dd($photos->toSql());
    ...
    public function update(UpdateGroupRequest $request, Group $group)
    {
        ...
        return back()->with('success', "Le groupe a été mis à jour");
    }
    ...
    public function updateImage(Request $request, Group $group)
    {
        ...
        if ($thumbnail) {
            ...
            $success = 'Votre image miniature a été mise à jour';
        }
...
//        session('success', 'Cover image has been updated');
    ...
    public function inviteUsers(InviteUsersRequest $request, Group $group)
    {
        ...
        return back()->with('success', 'L\'utilisateur a été invité à rejoindre le groupe');
    }
    ...
    public function approveInvitation(string $token)
    {
        ...
        return redirect(route('group.profile', $groupUser->group))
            ->with('success', 'Vous avez accepté de rejoindre le groupe "' . $groupUser->group->name . '"');
    }
    ...
    public function join(Group $group)
    {
        ...
        $status = GroupUserStatus::APPROVED->value;
        $successMessage = 'Vous avez rejoint le groupe "' . $group->name . '"';
        if (!$group->auto_approval) {
            $status = GroupUserStatus::PENDING->value;

            Notification::send($group->adminUsers, new RequestToJoinGroup($group, $user));
            $successMessage = 'Votre demande a été acceptée. Vous serez notifié une fois que vous serez approuvé';
        }
    ...
    public function removeUser(Request $request, Group $group)
    {
        ...
        if ($group->isOwner($user_id)) {
            return response("Le propriétaire du groupe ne peut pas être supprimé", 403);
        }
    ...
    public function changeRole(Request $request, Group $group)
    {
        ...
        $user_id = $data['user_id'];
        if ($group->isOwner($user_id)) {
            return response("Vous ne pouvez pas modifier le rôle du propriétaire du groupe", 403);
        }
