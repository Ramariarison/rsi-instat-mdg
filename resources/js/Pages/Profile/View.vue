            ...
            <div class="px-4">
                ...
                <div class="group relative bg-white dark:bg-slate-950 dark:text-gray-100">
                    ...
                    <div class="absolute top-2 right-2 ">
                        ...
                        <div v-else-if="isMyProfile" class="flex gap-2 bg-white p-2 opacity-0 group-hover:opacity-100">
                            <button
                    ...
                    <div class="flex">
                        <div
                            ...
                            <div v-else-if="isMyProfile" class="absolute top-1 right-0 flex flex-col gap-2">
                                <button
                        ...
                        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center flex-1 p-4 gap-4">
                            <div>
                                <h2 class="font-bold text-lg">{{ user.name }}</h2>
                                <p class="text-xs text-gray-500">
                                    {{ followerCount }} Suivi(s) - {{ user.division ? user.division.nom : 'Division non attribué' }}
                                </p>
                            </div>

                            <div v-if="!isMyProfile" class="self-start sm:self-auto">
                ...
                <TabGroup>
                    <TabList class="flex bg-white dark:bg-slate-950 dark:text-white">
                        <Tab v-slot="{ selected }" as="template">
                            <TabItem text="Pub(s)" :selected="selected"/>
                        </Tab>
                        <Tab v-slot="{ selected }" as="template">
                            <TabItem text="Suivi(s)" :selected="selected"/>
                        </Tab>
                        <Tab v-slot="{ selected }" as="template">
                            <TabItem text="Abonnements" :selected="selected"/>
                        </Tab>
                        <Tab v-slot="{ selected }" as="template">
                            <TabItem text="Photos" :selected="selected"/>
                        </Tab>
                        <Tab v-if="isMyProfile" v-slot="{ selected }" as="template">
                            <TabItem text="Profil" :selected="selected"/>
                        </Tab>
                    </TabList>
                    ...
                    <TabPanels class="mt-2">
                        <TabPanel>
                            ...
                            <div v-else class="py-8 text-center dark:text-gray-100">
                                Vous n'avez pas la permission de voir ces publications.
                            </div>
                        </TabPanel>
                        <TabPanel>
                            <div class="mb-3">
                                <TextInput :model-value="searchFollowersKeyword" placeholder="Tapez pour rechercher"
                                           class="w-full"/>
                            </div>
                            ...
                            <div v-else class="text-center py-8">
                                L'utilisateur n'a pas d'abonnés.
                            </div>
                        </TabPanel>
                        <TabPanel>
                            <div class="mb-3">
                                <TextInput :model-value="searchFollowingsKeyword" placeholder="Tapez pour rechercher"
                                           class="w-full"/>
                            </div>
                            ...
                            <div v-else class="text-center py-8">
                                L'utilisateur n'est abonné à personne.
                            </div>
                        </TabPanel>
                        <TabPanel>
