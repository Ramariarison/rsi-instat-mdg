...
function deleteComment(comment) {
    if (!window.confirm('Vous etes sur de vouloir supprimer cet commentaire?')) {
        return false;
    }
...
<template>
    <div v-if="authUser" class="flex gap-2 mb-3">
        ...
        <div class="flex flex-1">
            <InputTextarea v-model="newCommentText" placeholder="Entrez votre commentaire ici" rows="1"
                           class="w-full max-h-[160px] resize-none rounded-r-none"></InputTextarea>
            <IndigoButton @click="createComment" class="rounded-l-none w-[100px] ">Soumettre</IndigoButton>
        </div>
    </div>
    <div>
        <div v-for="comment of data.comments" :key="comment.id" class="mb-4">
            ...
            <div class="pl-12">
                <div v-if="editingComment && editingComment.id === comment.id">
                    <InputTextarea v-model="editingComment.comment" placeholder="Entrez votre commentaire ici"
                                   rows="1" class="w-full max-h-[160px] resize-none"></InputTextarea>

                    <div class="flex gap-2 justify-end">
                        <button @click="editingComment = null" class="rounded-r-none text-indigo-500">annuler
                        </button>
                        <IndigoButton @click="updateComment" class="w-[100px]">mettre à jour
                        </IndigoButton>
                    </div>
                </div>
                ...
                <Disclosure>
                    <div class="mt-1 flex gap-2">
                        <button @click="sendCommentReaction(comment)"
                            ...
                            {{ comment.current_user_has_reaction ? 'ne pas aimer' : 'aimer' }}
                        </button>
                        <DisclosureButton
